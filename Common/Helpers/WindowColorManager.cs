...
public static partial class WindowColorManager
{
    ...
    private static partial void InvalidateRect(IntPtr hWnd, IntPtr lpRect, [MarshalAs(UnmanagedType.Bool)] bool bErase);

    [LibraryImport("user32.dll")]
    [return: MarshalAs(UnmanagedType.Bool)]
    private static partial bool RedrawWindow(IntPtr hWnd, IntPtr lprcUpdate, IntPtr hrgnUpdate, uint flags);
    ...
    [LibraryImport("user32.dll")]
    private static partial void SendMessage(IntPtr hWnd, int msg, IntPtr wParam, IntPtr lParam);
    #endregion
    ...
    {
        if (window == null) return new WindowColorResult(false, false, "None", "窗口为空");

        var actualTitleBarColor = titleBarColor ?? borderColor;
        var hwnd = new WindowInteropHelper(window).Handle;
        if (hwnd == IntPtr.Zero) return new WindowColorResult(false, false, "None", "无法获取窗口句柄");

        // Windows 11: 使用原生 API
        if (IsWindows11OrLater()) return SetWindows11Colors(hwnd, borderColor, actualTitleBarColor);

        // Windows 10: 使用组合方法
        var unsupportedResult = new WindowColorResult(false, false, "Unsupported", "不支持的系统版本");
        if (!IsWindows10()) return unsupportedResult;
    ...
    public static bool RefreshWindow(Window window)
    {
        // 强制重绘窗口
        var hwnd = new WindowInteropHelper(window).Handle;
        if (hwnd == IntPtr.Zero) return false;
        const uint RDW_FRAME = 0x0400;// 重绘窗口边框
        const uint RDW_INVALIDATE = 0x0001;// 重绘窗口内容
        const uint RDW_UPDATENOW = 0x0100;// 立即更新窗口
        const uint RDW_ALLCHILDREN = 0x0080; // 重绘所有子窗口
