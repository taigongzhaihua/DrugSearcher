...
public partial class CodeCompletionService : IDisposable
{
    ...
    private void UpdateDeclaredVariables(string code)
    {
        lock (_lock)
        {
            ...
            {
                if (param.Name != null) _declaredVariables.Add(param.Name);
            }

            // 提取代码中声明的变量
            var codeVariables = ExtractCodeVariables(code);
            foreach (var variable in codeVariables.OfType<string>())
            {
    ...
    private static partial Regex FuncMatchesRegex();
}
    ...
    : JavaScriptCompletionDataProvider
{
    ...
    private readonly List<DosageParameter> _parameters = [];
