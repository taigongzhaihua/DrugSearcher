...
public partial class JavaScriptSyntaxHighlightingGenerator
{
    ...
    public static IHighlightingDefinition GenerateDefinition(bool isDarkTheme)
    {
        ...
        foreach (var namedColor in namedColors.Values)
        {
            definition.NamedHighlightingColorsList.Add(namedColor);
        }
    ...
    private static Dictionary<string, HighlightingColor> CreateNamedColors(Dictionary<string, Color> colors)
    {
        ...
        foreach (var (key, value) in colors)
        {
            ...
            {
                Name = key,
                Foreground = new SimpleHighlightingBrush(value)
            };
            ...
            namedColors[key] = highlightingColor;
        }
    ...
    private static void AddMultilineCommentSpan(HighlightingRuleSet? ruleSet, HighlightingColor color)
    {
        ...
        {
            StartExpression = MultilineCommentStartRegex(),
            EndExpression = MultilineCommentEndRegex(),
            SpanColor = color,
    ...
    private static void AddStringSpans(HighlightingRuleSet? ruleSet, HighlightingColor color)
    {
        ...
        {
            StartExpression = DoubleQuoteStartRegex(),
            EndExpression = DoubleQuoteEndRegex(),
            SpanColor = color,
            SpanColorIncludesStart = true,
            SpanColorIncludesEnd = true
        };
        ...
        {
            StartExpression = SingleQuoteStartRegex(),
            EndExpression = SingleQuoteEndRegex(),
            SpanColor = color,
            SpanColorIncludesStart = true,
            SpanColorIncludesEnd = true
        };
        ...
        {
            StartExpression = TemplateStringStartRegex(),
            EndExpression = TemplateStringEndRegex(),
            SpanColor = color,
            SpanColorIncludesStart = true,
            SpanColorIncludesEnd = true
        };
    ...
    private static void AddKeywordRules(HighlightingRuleSet? ruleSet, HighlightingColor color)
    {
        ...
        {
            Color = color,
            Regex = new Regex(keywordPattern, RegexOptions.Compiled)
        });
    }
    ...
    {
        // 内置函数
        var builtInFunctions = JavaScriptLanguageDefinition.BuiltIns.GlobalFunctions.Keys.ToList();
        if (builtInFunctions.Count != 0)
        {
            ...
            {
                Color = color,
                Regex = new Regex(functionPattern, RegexOptions.Compiled)
            });
        }

        // 内置对象
        var builtInObjects = JavaScriptLanguageDefinition.BuiltIns.GlobalObjects.Keys.ToList();
        if (builtInObjects.Count != 0)
        {
            ...
            {
                Color = color,
                Regex = new Regex(objectPattern, RegexOptions.Compiled)
            });
        }
    }
    ...
    private static void AddCustomFunctionRules(HighlightingRuleSet? ruleSet, HighlightingColor color)
    {
        ...
        {
            Color = color,
            Regex = new Regex(customFunctionPattern, RegexOptions.Compiled)
        });
    }
    ...
    private static void AddOperatorRule(HighlightingRuleSet? ruleSet, HighlightingColor color) =>
        ...
        {
            Color = color,
            Regex = new Regex(JavaScriptLanguageDefinition.Operators.AllCharacters, RegexOptions.Compiled)
        });
    ...
    [GeneratedRegex("\"")]
    private static partial Regex DoubleQuoteStartRegex();

    [GeneratedRegex("(?<!\\\\)\"")]
    private static partial Regex DoubleQuoteEndRegex();

    [GeneratedRegex("'")]
    private static partial Regex SingleQuoteStartRegex();

    [GeneratedRegex(@"(?<!\\)'")]
    private static partial Regex SingleQuoteEndRegex();

    [GeneratedRegex("`")]
    private static partial Regex TemplateStringStartRegex();

    [GeneratedRegex(@"(?<!\\)`")]
    private static partial Regex TemplateStringEndRegex();
...
internal class CustomHighlightingDefinition(string name) : IHighlightingDefinition
{
    ...
    public HighlightingColor? GetNamedColor(string colorName) =>
        NamedHighlightingColorsList.FirstOrDefault(c => c.Name == colorName);

    public HighlightingRuleSet? GetNamedRuleSet(string ruleName) => null;
}
